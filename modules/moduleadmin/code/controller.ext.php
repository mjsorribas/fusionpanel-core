...
class module_controller extends ctrl_module
{
    ...
    static function doInstallModule()
    {
        ...
        if ($_FILES['modulefile']['error'] > 0) {
            ...
            $module_dir = ctrl_options::GetSystemOption('fusionpanel_root') . 'modules/' . $module_folder;
            if (!fs_director::CheckFolderExists($module_dir)) {
                if ($archive_ext != 'zpp') {
                    self::$error_message = "Package type was not detected as a .zpp (Fusionpanel Package) archive.";
                } else {
